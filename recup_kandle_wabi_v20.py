...
def populate_indicators(btc_df) :
    ...
    EMA15 = dataframe['low'].ewm(span = 5, adjust = False).mean()
...
def ULTIMATE_NINJA_NEW(dataframe,val_mask_idxmin) :
        ...
        # if last_index >= 5 :
        #     VOLUME_PRECEDENT=dataframe.volume.iloc[-5].astype(int)
        #     VOLUME_COURANT=dataframe.volume.iloc[-1].astype(int)
        #     Diff_VOLUME_COURANT_VOLUME_PRECEDENT=(int(VOLUME_COURANT)-int(VOLUME_PRECEDENT))
       ...
       # if  int(DIFF_EMA200_EMA15_MOINS_1) < 0 and int(VARIATION_EMA200_EMA15) >= 0:
        ...
        if (COMPTEUR_ACHAT_VENTE >= 0):
            ...
            Len_Prev_index=len(dataframe)-2
            Len_last_index=len(dataframe)-1
            Prev_index=dataframe['SAR'].loc[Len_Prev_index].astype(float)
            VAR_last_index=dataframe['SAR'].loc[Len_last_index].astype(float)
            ...
            TEST_VARIATION_POSITIVE=VARIATION > 0
            last_index=len(dataframe)-1
           ...
           # !!!!!!!!!!!!!! DEB ACHAT 
            ...
            if (LAST_ORDER != "BUY") and  (POINT_ENTRE_EMA200 == 0) and (( FRONT_MONTANT_EMA1L == 1) or ( FRONT_MONTANT_EMA1H == 1)) and (FRONT_MONTANT_COURT_EMA15 == 1):
             if  (DIFF_SAR_SUP_PRIX == False) and (TYPE_BG == "BG_VERTE" or float(TAILLE_BG_BIS) >= 0)  :
              ...
              # 0000000000000000.................00333++++++++ if ((DIFF_NEW_HIGH_PRICE_SUP_NEW_OPEN_PRICE == True ) and (DIFF_VAL_CLOSE_INF_VAL_CLOSE_N2 == False) and (float(TAILLE_BG) > 0)) \
            ...
            if (LAST_ORDER != "SELL")   and  (POINT_ENTRE_EMA200 == 1) and GAIN1 > 0   and ( FRONT_MONTANT_EMA1H == 0) :
                 if (  float(GAIN1) > 0 and (VAL_EMA15_COURANT < VAL_OPEN) ) :
            ...
            # if (LAST_ORDER != "SELL")   and  (POINT_ENTRE_EMA200 == 1) and float(GAIN1) > 0 : 
            if (LAST_ORDER != "SELL")   and  (POINT_ENTRE_EMA200 == 1)  :
                if ( int(COMPTEUR_DIFF_GAIN1_EGAL) >= int(NBR_COMPTEUR_DIFF_GAIN1) ) :
                  # if (  float(GAIN1) > 0 and (TYPE_BG_BIS == "BG_VERTE")) \
                  if (  float(GAIN1) > 0 and (TYPE_BG == "BG_ROUGE") or (TYPE_BG_BIS == "BG_ROUGE") and int(GAIN1) >= 0) \
                      or (last_index - INDEXMAX == 1  and DIFF_SAR_SUP_PRIX==True and int(GAIN1) <= 0) :
    ...
    if (ORDER == "SELL") and (LAST_ORDER == "BUY") or (DIFF_DYNAMIQUE_VAL_CLOSE_VAL_CLOSE_ORG == True) :
           ...
           if (GAIN_RELATIF >= 0):
               if COMPTEUR_SELL == 0 :
                  ...
                  CUMUL_GAIN_RELATIF_OLD=float(custom_info["CUMUL_GAIN_RELATIF"])
